...
export default function CartPage() {
  ...
  return (
    <div className="min-h-screen bg-brand-cream">
      <PageHero title="Shopping Cart" backgroundImage="/hero/hero-shop.png" />
        ...
        {cartItems.length === 0 && savedItems.length === 0 ? (
          <section className="py-20">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 text-center">
              ...
              <p className="text-brand-cocoa/70 mb-8 text-lg">Looks like you haven&apos;t added anything to your cart yet</p>
                ...
                <div className="lg:col-span-2 space-y-6">
                  <div className="bg-white/90 rounded-2xl shadow-luxury border border-brand-nude/50 p-6 overflow-hidden">
                    ...
                    <div className="space-y-6">
                      {cartItems.map((item) => (
                        <div key={`${item.id}-${item.variant || ''}`} className="flex flex-col sm:flex-row gap-4 sm:gap-6 pb-6 border-b border-brand-nude/50 last:border-0 last:pb-0">
                          <Link href={`/product/${item.slug || item.id}`} className="relative w-full sm:w-32 h-48 sm:h-32 flex-shrink-0 bg-brand-nude/30 rounded-2xl overflow-hidden">
                          ...
                          <div className="flex-1">
                            <div className="flex justify-between mb-2">
                              <Link href={`/product/${item.slug || item.id}`} className="text-lg font-semibold text-brand-espresso hover:text-brand-mauve transition-colors line-clamp-2">
                                {item.name}
                              </Link>
                              <button
                            ...
                            <div className="text-sm text-brand-cocoa/70 mb-3 space-y-1">
                              {item.variant && <p>Variant: {item.variant}</p>}
                              <p className="text-brand-espresso font-medium">In Stock</p>
                            </div>

                            <div className="flex items-center justify-between flex-wrap gap-4">
                              <div className="flex items-baseline space-x-3">
                                <span className="font-display text-xl font-semibold text-brand-espresso">GH₵{item.price.toFixed(2)}</span>
                              </div>
                                ...
                                <div className="flex flex-col">
                                  <div className="flex items-center border-2 border-brand-nude/70 rounded-xl overflow-hidden">
                                    <button
                                      ...
                                      className="w-10 h-10 flex items-center justify-center text-brand-cocoa hover:bg-brand-nude/30 transition-colors cursor-pointer"
                                    ...
                                    <input
                                      ...
                                      className="w-12 h-10 text-center border-x-2 border-brand-nude/70 focus:outline-none font-semibold text-brand-espresso"
                                      min={item.moq || 1}
                                      max={item.maxStock}
                                    />
                                    <button
                            ...
                            <button
                              ...
                              className="mt-3 text-sm text-brand-espresso hover:text-brand-espresso font-medium whitespace-nowrap"
                            >
                              Save for Later
                            </button>
                            */}
                  ...
                  {savedItems.length > 0 && (
                    <div className="bg-white/90 rounded-2xl shadow-luxury border border-brand-nude/50 p-6">
                      ...
                      <div className="space-y-4">
                        {savedItems.map((item) => (
                          <div key={item.id} className="flex gap-4 pb-4 border-b border-brand-nude/50 last:border-0 last:pb-0">
                            <div className="relative w-20 h-20 flex-shrink-0 bg-brand-nude/30 rounded-xl overflow-hidden">
                            ...
                            <div className="flex-1">
                              ...
                              <p className="font-display text-lg font-semibold text-brand-espresso mb-2">GH₵{item.price.toFixed(2)}</p>
                ...
                <div className="space-y-6">
                  <div className="bg-white/90 rounded-2xl shadow-luxury border border-brand-nude/50 p-6 sticky top-24">
                    ...
                    <div className="space-y-4 mb-6">
                      <div className="flex justify-between text-brand-cocoa/80">
                        <span>Subtotal</span>
                        <span className="font-semibold">GH₵{subtotal.toFixed(2)}</span>
                      </div>

                      {appliedCoupon && (
                        <div className="flex justify-between text-brand-mauve">
                    ...
                    <div className="border-t border-brand-nude/50 pt-4 mb-6">
                      <div className="flex justify-between text-xl font-semibold text-brand-espresso">
                        <span>Total</span>
                    ...
                    <Link
                      href="/checkout"
                      className="block w-full btn-luxury-primary py-4 text-center mt-6 mb-3"
                    >
                      Proceed to Checkout
                    </Link>
                    ...
                    <Link
                      href="/shop"
                      className="block w-full text-center text-brand-mauve hover:text-brand-espresso font-semibold py-2 whitespace-nowrap transition-colors"
                    >
                      Continue Shopping
                    </Link>

                    <div className="mt-6 pt-6 border-t border-brand-nude/50 space-y-3">
                      <div className="flex items-center text-sm text-brand-cocoa/70">
                        <i className="ri-shield-check-line text-brand-espresso mr-2"></i>
