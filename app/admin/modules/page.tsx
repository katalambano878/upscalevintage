...
const colorMap: Record<string, string> = {
  ...
  blue: 'bg-store-surface text-store-muted',
  purple: 'bg-store-primary/15 text-store-primary',
    ...
    return (
      <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-8 text-center border border-gray-100">
          <div className="w-20 h-20 bg-store-surface rounded-full flex items-center justify-center mx-auto mb-6">
            <i className="ri-lock-2-line text-4xl text-store-muted"></i>
          </div>
          ...
          <form onSubmit={handleUnlock} className="space-y-4">
            <div>
              <input
                ...
                className="w-full text-center text-3xl font-bold tracking-widest px-4 py-4 border-2 border-gray-200 rounded-xl focus:border-store-primary focus:ring-4 focus:ring-store-primary/10 outline-none transition-all"
                placeholder="• • • •"
                maxLength={4}
                autoFocus
              />
            ...
            <button
              type="submit"
              className="w-full bg-gray-900 hover:bg-store-navy text-white font-bold py-4 rounded-xl transition-colors text-lg"
            >
              Unlock Dashboard
            </button>
          </form>
          ...
          <div className="flex gap-4">
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-store-primary outline-none"
              />
            </div>
            <select
        ...
        {loading ? (
          ...
          <div className="space-y-10">
            {Object.entries(groupedModules).map(([category, items]) => (
              <div key={category}>
                <h2 className="text-xl font-bold text-gray-800 mb-4 border-l-4 border-store-muted pl-3">{category}</h2>
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {items.map(module => (
                    <div key={module.id} className={`bg-white rounded-xl border-2 p-6 transition-all ${module.enabled ? 'border-store-muted shadow-md' : 'border-gray-200 opacity-75'}`}>
                      <div className="flex justify-between items-start mb-4">
                        ...
                        <button
                          ...
                          className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${module.enabled ? 'bg-store-navy-light' : 'bg-gray-300'}`}
                        >
                      ...
                      <div className="mt-4 pt-4 border-t border-gray-100 flex justify-between items-center">
                        <span className={`text-xs font-bold px-2 py-1 rounded-full ${module.enabled ? 'bg-store-surface text-store-ink' : 'bg-gray-100 text-gray-600'}`}>
