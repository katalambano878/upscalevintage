...
export default function ProductsPage() {
  ...
  const fetchCategories = useCallback(async () => {
    try {
      const data = await apiData<{ name: string }[]>('/api/catalog/categories');
  ...
  const handleDeleteProduct = async (productId: string) => {
    ...
    try {
      await apiDelete(`/api/catalog/products/${productId}`);
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <Link
          ...
          className="px-6 py-3 bg-store-navy hover:bg-store-navy text-white rounded-lg font-semibold transition-colors whitespace-nowrap cursor-pointer flex items-center justify-center md:items-start"
        >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-xl border-2 border-gray-200 p-4">
          ...
          <p className="text-2xl font-bold text-store-ink">{stats.active}</p>
        </div>
        <div className="bg-white rounded-xl border-2 border-gray-200 p-4">
            ...
            <div className="flex items-center space-x-3">
              ...
              <div className="flex border-2 border-gray-300 rounded-lg overflow-hidden">
                <button
                  ...
                  className={`w-10 h-10 flex items-center justify-center transition-colors cursor-pointer ${viewMode === 'list' ? 'bg-store-navy text-white' : 'bg-white text-gray-700 hover:bg-gray-50'
                    }`}
                >
                ...
                <button
                  ...
                  className={`w-10 h-10 flex items-center justify-center border-l-2 border-gray-300 transition-colors cursor-pointer ${viewMode === 'grid' ? 'bg-store-navy text-white' : 'bg-white text-gray-700 hover:bg-gray-50'
                    }`}
                >
              ...
              <div key={product.id} className="border-2 border-gray-200 rounded-xl p-4 hover:shadow-lg transition-shadow">
                <div className="relative">
                  <input
                    ...
                    checked={selectedProducts.includes(product.id)}
                    onChange={() => handleSelectProduct(product.id)}
                    className="absolute top-2 left-2 w-5 h-5 text-store-ink border-gray-300 rounded focus:ring-store-primary cursor-pointer z-10"
                  />
                ...
                <div className="flex items-center space-x-2">
                  <Link
                    href={`/admin/products/${product.id}`}
                    onClick={persistListState}
                    className="flex-1 bg-store-navy hover:bg-store-navy text-white py-2 rounded-lg text-sm font-medium text-center transition-colors whitespace-nowrap cursor-pointer"
                  >
                    Edit
                  </Link>
                  <button
