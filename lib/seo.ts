...
const CANONICAL_SITE_URL = 'https://upscalevintage.shop';

/**
 * Resolve the canonical site URL.
 *
 * Order of preference:
 * 1. `NEXT_PUBLIC_APP_URL` if set AND not a localhost value when running in
 *    production (Vercel deploys with a stale localhost env var were emitting
 *    `http://localhost:3000` into the live sitemap — Google Search Console
 *    rejects those with "URL not allowed").
 * 2. The Vercel auto-injected `VERCEL_PROJECT_PRODUCTION_URL` for prod, or
 *    `VERCEL_URL` for preview deployments.
 * 3. `CANONICAL_SITE_URL` as the hard fallback so we *never* ship localhost.
 *
 * In local development we still respect `NEXT_PUBLIC_APP_URL=http://localhost:3000`
 * so OG previews, callbacks, etc. work.
 */
function resolveSiteUrl(): string {
  ...
  if (vercelPreview && !isProd) return `https://${vercelPreview.replace(/\/$/, '')}`;
...
> = {
  ...
  help: {
    ...
    description:
      'Need help with an order, return, payment, or account? Browse Upscale Vintage support articles or contact our team in Accra, Ghana.',
    keywords: ['help center', 'customer support Ghana', 'Upscale Vintage support'],
    path: '/help',
  },
  shipping: {
...
export function articleJsonLd(article: {
  ...
  return {
    ...
    publisher: {
      ...
      logo: {
        '@type': 'ImageObject',
        url: absoluteImage(SEO_ASSETS.logo),
      },
    },
